...
import { BIN_STATE_DIR, LOCAL_BIN_DIR } from "./lib/paths";
...
async function managedStatus(command: "find" | "grep"): Promise<string> {
  ...
  return content.includes(`cmd-bridge-managed: ${command}`)
    ? `${command}: managed`
...
async function main(): Promise<void> {
  ...
  const runner = path.join(BIN_STATE_DIR, "cmd-bridge-runner.mjs");

  console.log("cmd-bridge doctor");
